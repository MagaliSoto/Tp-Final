    ...
    def reescalar_imagenes(lista_animaciones, W, H):
        for lista in lista_animaciones:
            for i in range(len(lista)):
                imagen = lista[i]
                num_columnas = imagen.get_width() // W
                num_filas = imagen.get_height() // H
                animaciones = []
                for fila in range(num_filas):
                    for columna in range(num_columnas):
                        x = columna * W
                        y = fila * H
                        cuadro = imagen.subsurface(pygame.Rect(x, y, W, H))
                        animaciones.append(pygame.transform.scale(cuadro, (W, H)))
                lista[i] = animaciones
